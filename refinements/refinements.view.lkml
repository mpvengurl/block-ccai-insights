...
view: +insights_data {
  #Configure manual score and magnitude thresholds here in the default_value field or expose the field within the UI time experimentatiodocumentation here: https://cgoogle.com/natural-language/docs/basics#interpreting_sentiment_analysis_values
  ...
  dimension: client_sentiment_category {
    ...
    type: string
    description: "Sentiment category of the client's interactions. Sentiment is comprised of both Score (overall emotion) and Magnitude (how much emotional content is present). Positive is a positive score and high magnitude, Negative is negative score and high magnitude, Mixed is neutral score and high magnitude, and Neutral is neutral score and neutral/low magnitude."
    sql: CASE
          ...
          WHEN (${client_sentiment_score} < {% parameter sentiment_score_threshold %} OR ${client_sentiment_score} > -{% parameter sentiment_score_threshold %}) AND ${client_sentiment_magnitude} > {% parameter sentiment_magnitude_threshold %} THEN "Mixed"
          ELSE "Neutral" END;;
  }
  ...
  dimension: agent_sentiment_category {
    group_label: "Sentiment"
    type: string
    description: "Sentiment category of the agent's interactions. Sentiment is comprised of both Score (overall emotion) and Magnitude (how much emotional content is present). Positive is a positive score and high magnitude, Negative is negative score and high magnitude, Mixed is neutral score and high magnitude, and Neutral is neutral score and neutral/low magnitude."
    sql: CASE
          ...
          WHEN (${agent_sentiment_score} < {% parameter sentiment_score_threshold %} OR ${agent_sentiment_score} > -{% parameter sentiment_score_threshold %}) AND ${agent_sentiment_magnitude} > {% parameter sentiment_magnitude_threshold %} THEN "Mixed"
          ELSE "Neutral" END;;
  }
